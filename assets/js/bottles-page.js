...
function notesToHtml(raw) {
  const s = String(raw ?? "");
  if (!s) return "";
  const withoutBrackets = s.replace(/\[\[([\s\S]*?)\]\]/g, "$1");
  return escapeHtml(withoutBrackets);
}
          ...
          items.length
            ? items
                .map((it) => {
                  ...
                  // ✅ ID removed from display; make the bottle name clickable instead
                  const titleHtml = id
                    ? `<a class="foe-link" href="${escapeHtml(href)}" data-barrel-link="1" data-barrel-id="${escapeHtml(id)}">${escapeHtml(leftTitle)}</a>`
